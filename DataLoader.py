...
class DataLoader(object):
    ...
    def batch_iter(self, data, batch_size, shuffle):
        ...
        train_summary_file = open('original_data/train.summary', 'r').read().strip().split('\n')
        train_summary_toks = [list(summary.strip().split(' ')) for summary in train_summary_file]
        ...
        if shuffle:
            ...
            train_summary_toks = np.array(train_summary_toks)[shuffle_indices]
        ...
        for batch_num in range(num_batches):
            ...
            batch_data = {'enc_in':[], 'enc_fd':[], 'enc_pos':[], 'enc_rpos':[], 'enc_len':[],
                          'dec_in':[], 'dec_len':[], 'dec_out':[], 'summary_toks':[]}

            for summary, train_summary_tok, text, field, pos, rpos in zip(summaries[start_index:end_index], 
                                            train_summary_toks[start_index:end_index], 
                ...
                batch_data['summary_toks'].append(train_summary_tok)
